...
{
    [Header("Black Hole Influence")]
    public Transform blackHole;
    public float blackHoleRadius = 20f;
    public float blackHoleInfluence = 2f;
    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            Vector3 toHole = Vector3.zero;
            float influence = 0f;

            if (blackHole != null && Vector3.Distance(transform.position, blackHole.position) <= blackHoleRadius)
            {
                toHole = (blackHole.position - playerCamera.transform.position).normalized;
                float dot = Vector3.Dot(toHole, playerCamera.transform.forward);
            ...
            influence = attractionCurve.Evaluate(gazeTimer);

            // Sensitivity wordt beïnvloed
            mouseSensitivity = Mathf.Lerp(defaultSensitivity, minSensitivity, influence);

            // Bereken standaard input
            ...
            // Voeg extra aantrekkingsrotatie toe
            if (blackHole != null)
            {
                Quaternion toBlackHoleRot = Quaternion.LookRotation(toHole);
                Vector3 targetEuler = toBlackHoleRot.eulerAngles;

                float targetYaw = targetEuler.y;
                float targetPitch = -targetEuler.x;

                yaw = Mathf.LerpAngle(yaw, targetYaw, Time.deltaTime * blackHoleInfluence * influence);
                pitch = Mathf.LerpAngle(pitch, targetPitch, Time.deltaTime * blackHoleInfluence * influence);
            }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.Space();
        GUILayout.Label("Black Hole Influence", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 });

        fpc.blackHole = (Transform)EditorGUILayout.ObjectField(new GUIContent("Black Hole", "Het object waarnaar de speler aangetrokken wordt."), fpc.blackHole, typeof(Transform), true);
        fpc.blackHoleRadius = EditorGUILayout.Slider(new GUIContent("Black Hole Radius", "Hoe dichtbij je moet zijn voor effect."), fpc.blackHoleRadius, 1f, 100f);
        fpc.blackHoleInfluence = EditorGUILayout.Slider(new GUIContent("Attraction Strength", "Hoe sterk je ernaartoe wordt gedraaid."), fpc.blackHoleInfluence, 0f, 10f);
        fpc.minSensitivity = EditorGUILayout.Slider(new GUIContent("Minimum Sensitivity", "Hoe traag de muis mag worden."), fpc.minSensitivity, 0.01f, fpc.mouseSensitivity);
        fpc.attractionCurve = EditorGUILayout.CurveField(new GUIContent("Attraction Curve", "Hoe snel de aantrekkingskracht toeneemt."), fpc.attractionCurve);
